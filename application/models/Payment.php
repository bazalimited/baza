...
class Payment extends CI_Model {
    ...
    function get_all($limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc') {
		...
		location_items.unit_price as location_unit_price, phppos_locations.name as locationa_name');
        ...
        $this->db->join('phppos_locations', 'phppos_items.item_location_id = phppos_locations.location_id', 'left');
        $this->db->group_by('items.item_id');
        $this->db->where('items.deleted', 0);
        $this->db->where('location_items.location_id', $current_location);
    ...
    function get_payment_by_item_id($item_id, $item_type) {
        $this->db->from('payments');
        $this->db->where('item_id', $item_id);
        $this->db->where('item_type', $item_type);
        $this->db->where('payment_gatway_code', SUCCESS_STATUS_CODE);
    ...
    function get_all_payment_by_item_id($item_id, $item_type) {
        $this->db->from('payments');
        $this->db->where('item_id', $item_id);
        $this->db->where('item_type', $item_type);
        //$this->db->where('payment_gatway_code', SUCCESS_STATUS_CODE);
    ...
    function get_pending_payments_by_item_id($item_id, $item_type) {
        $this->db->from('payments');
        $this->db->where('item_id', $item_id);
        $this->db->where('item_type', $item_type);
        $this->db->where('payment_gatway_code in(' . PENDING_STATUS_CODE . ', ' . PENDING_COMMIT_TO_WALLET_STATUS_CODE . ')');
        $this->db->where('deleted', 0);
        $result = $this->db->get();
    ...
    function get_all_by_user($user_id, $limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc') {

        $locations_list = $this->Employee->get_employee_locations($user_id);
        $location_ids = array();
        if (count($locations_list) > 0) {
            foreach ($locations_list as $key => $value) {
                array_push($location_ids, $key);
            }
        }
        $current_location = $this->Employee->get_logged_in_employee_current_location_id();
        $this->db->select('items.*, GROUP_CONCAT(' . $this->db->dbprefix('tags') . '.name) as tags, categories.name as category,
        ...
        if (count($location_ids) > 0) {
            $this->db->where_in($this->db->dbprefix('items') . '.item_location_id', $location_ids);
        } else {
            $this->db->where($this->db->dbprefix('items') . '.item_location_id', $current_location);
        }
        if (!$this->config->item('speed_up_search_queries')) {
            $this->db->order_by($col, $order);
        }
    ...
    function get_all_unpaid_by_user($user_id, $limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc', $agent_not_paid = true, $owner_not_paid = true) {
        ...
        if ($owner_not_paid) {
            $this->db->where('items.agent_payment_id is null');
        }
        if (!$this->config->item('speed_up_search_queries')) {
            $this->db->order_by($col, $order);
        }
    ...
    function count_all_unpaid_by_user($user_id, $limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc', $agent_not_paid = true, $owner_not_paid = true) {
    ...
    function count_all_by_user($user_id, $limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc') {
    ...
    function get_all_by_category_and_location($category_id, $hide_out_of_stock_grid = true, $offset = 0, $limit = 14) {
        $items_table = $this->db->dbprefix('items');
        $item_kits_table = $this->db->dbprefix('item_kits');

        $location_items_table = $this->db->dbprefix('location_items ');
        $current_location = $this->Employee->get_logged_in_employee_current_location_id();
        $result = $this->db->query("(SELECT i.item_id, i.unit_price, name,size, image_id FROM $items_table as i JOIN $location_items_table as li ON i.item_id = li.item_id and li.location_id = $current_location
			WHERE deleted = 0 and category_id = $category_id ORDER BY name) UNION ALL (SELECT CONCAT('KIT ',item_kit_id), unit_price, name, '', 'no_image' as image_id FROM $item_kits_table 
			WHERE deleted = 0 and category_id = $category_id ORDER BY name) ORDER BY name LIMIT $offset, $limit");
    ...
    function test_items($category_id, $hide_out_of_stock_grid = true, $offset = 0, $limit = 14) {
        $location_items_table = $this->db->dbprefix('location_items ');
        $current_location = $this->Employee->get_logged_in_employee_current_location_id();
        echo "(SELECT i.item_id, i.unit_price, name,size, image_id FROM phppos_items as i LEFT JOIN $location_items_table as li ON i.item_id = li.item_id and li.location_id = $current_location
			WHERE (quantity > 0 or quantity IS NULL) and deleted = 0 and category_id = $category_id ORDER BY name) UNION ALL (SELECT CONCAT('KIT ',item_kit_id), unit_price, name, '', 'no_image' as image_id FROM $item_kits_table 
			WHERE deleted = 0 and category_id = $category_id ORDER BY name) ORDER BY name LIMIT $offset, $limit";
    }
    ...
    function count_all_by_location() {
        $current_location = $this->Employee->get_logged_in_employee_current_location_id();
        $this->db->from('items');
...
//        exit();
    ...
    function get_payment_by_transaction_id($transaction_id) {
        $this->db->from('payments');
        $this->db->where('transaction_id', $transaction_id); // We may have may payment with same transaction id in case items was payed in bulk
        $this->db->where('deleted', 0);
        return $this->db->get();
    }
    ...
    function item_payment_saved($item_id, $item_type = 2) {
        $this->db->from('payments');
        $this->db->where('item_id', $item_id);
        $this->db->where('item_type', $item_type);
        $this->db->where('deleted', 0);
    ...
    function get_item_payment_id($item_id, $item_type = 2) {
        $this->db->from('payments');
        $this->db->where('item_id', $item_id);
        $this->db->where('item_type', $item_type);
        $this->db->where('deleted', 0);
    ...
    function get_item_payment($item_id, $item_type = 2) {
        $this->db->from('payments');
        $this->db->where('item_id', $item_id);
        $this->db->where('item_type', $item_type);
        $this->db->where('deleted', 0);
    ...
    function check_item_payment_status($item_id, $item_type) {
        $item_payment_status = $this->Item->item_payment_status($item_id);
        $item_payment = $this->Payment->get_payment_by_item_id($item_id, $item_type); // 1 for found items

        if ($item_payment_status == PENDING_STATUS_CODE || $item_payment_status == PENDING_COMMIT_TO_WALLET_STATUS_CODE) {
            if (is_object($item_payment)) { // at least we request a payment
                // check how old it is. if it is old than 1 hour, then we delete it so that the system can request another payment
                $t1 = time();
                $t2 = strtotime($item_payment->date_created);
                $diff = $t1 - $t2;
                $payment_old = round($diff / ( 60), 0); // in hour

                if (intval($payment_old) > 60) { // One hour old
                    //delete tha rqested sale
                    $this->Payment->delete_payment($item_payment->payment_id);
                } else {
                    $this->Payment->get_request_mobile_payment_status($item_payment->payment_id);
                }
            }
        } else {
            if ($item_payment_status == REJECTED_BY_THE_PAYER) {
                $payment_data = array('deleted' => 1);
                $this->Payment->save($payment_data, $item_payment->payment_id);
                //delete tha rqested sale
                //$this->Payment->delete_payment($item_payment->payment_id);
            }
        }
    ...
    function request_mobile_payment_approval($amount, $phone_number, $description, $transaction_id) {
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => "http://opay.rw/8081/payment",
            ...
            CURLOPT_POSTFIELDS => "{ \r\n  \"amount\": " . $amount . ", \r\n  \"description\": \"" . $description . "\", \r\n  \"merchantId\": \"eaa1a7f3b06b4beea9b1dad5f2db99bd\", \r\n  \"telephoneNumber\": \"" . $phone_number . "\", \r\n  \"transactionId\": \"" . $transaction_id . "\" \r\n}",
            CURLOPT_HTTPHEADER => array(
                "cache-control: no-cache",
                "content-type: application/json",
                "postman-token: 34cf1ed8-1876-03f1-4db0-665840b72a9b"
            ),
        ));
        ...
        curl_close($curl);
        $payments = $this->Payment->get_payment_by_transaction_id($transaction_id);

        if (!$err) {
            $response_array = json_decode($response, TRUE); // Set second argument as TRUE

            $request_code = $response_array['code'];
            $description = $response_array['description'];

            $payment_data = array('payment_gatway_code' => $request_code, 'payment_gatway_message' => $description);

            if (count($payments) > 0) {
                foreach ($payments->result() as $payment) {
                    $this->Payment->save($payment_data, $payment->payment_id);
                }
            }
            if (in_array($request_code, array(101, 102, 103))) {
                return true;
            }

//            else {
//                // delete the payment from the syastem
...
//            }
    ...
    function set_item_payment_check($item_id, $item_type) {
        if ($item_type == 2) {
            $item_payment_status = $this->Personal_item->item_payment_status($item_id);
        } else {
            $item_payment_status = $this->Item->item_payment_status($item_id);
        }
        $item_payment = $this->Payment->get_payment_by_item_id($item_id, $item_type); // 2 for personal items

        if (is_object($item_payment)) {
            if ($item_payment_status == PENDING_STATUS_CODE || $item_payment_status == PENDING_COMMIT_TO_WALLET_STATUS_CODE) {
                if (is_object($item_payment)) { // at least we request a payment
                    // check how old it is. if it is old than 1 hour, then we delete it so that the system can request another payment
                    $t1 = time();
                    ...
                    if (intval($payment_old) > 60) {
                        //delete tha rqested sale
                        $this->Payment->delete_payment($item_payment->payment_id);
                    } else {
                        $this->Payment->get_request_mobile_payment_status($item_payment->transaction_id);
                    }
                }
            } else {
                if ($item_payment_status == REJECTED_BY_THE_PAYER) {
    ...
    function delete_payment($payment_id) {
        ...
        $selected_payment = $this->Payment->get_info($payment_id);

        if (is_object($selected_payment)) {


            if ($selected_payment->item_type == 2) { // User registered item
                $item_data = array('owner_payment_id' => null);
                $this->Personal_item->save($item_data, $selected_payment->item_id);
            } else {
                if ($selected_payment->payment_type == 2) { //Owner Payment for found items
                    $item_data = array('owner_payment_id' => null);
                    $this->Item->save($item_data, $selected_payment->item_id);
                }
            }
            // delete the payment from the syastem
            $this->db->where('payment_gatway_code <> ' . SUCCESS_STATUS_CODE . ''); //if the payment is not confirmed
            $this->db->where('payment_id', $payment_id);
    ...
    function get_item_search_suggestions($search, $limit = 25, $location_id = 0) {
        if ($location_id == 0) {
    ...
    function search($search, $category_id = false, $limit = 20, $offset = 0, $column = 'name', $orderby = 'asc', $fields = 'all') {
        ...
        $this->db->join('phppos_locations', 'phppos_items.item_location_id = phppos_locations.location_id', 'left');
        $this->db->join('categories', 'categories.id = items.category_id', 'left');
        $this->db->join('location_items', 'location_items.item_id = items.item_id and location_items.location_id = ' . $current_location);
        ...
        if ($fields == 'all') {
            if ($search) {
                if ($this->config->item('supports_full_text') && !$this->config->item('legacy_search_method')) {
                    ...
                    foreach ($search_terms_array as $x) {
                        $sql_search_name_criteria .= ($search_name_criteria_counter > 0 ? " AND " : "") .
            ...
            if ($search) {
                //Exact Match fields
                if ($fields == $this->db->dbprefix('items') . '.item_id' || $fields == $this->db->dbprefix('items') . '.reorder_level' || $fields == $this->db->dbprefix('location_items') . '.quantity' || $fields == $this->db->dbprefix('items') . '.cost_price' || $fields == $this->db->dbprefix('items') . '.unit_price' || $fields == $this->db->dbprefix('items') . '.promo_price' || $fields == $this->db->dbprefix('tags') . '.name') {
    ...
    function search_items($search, $category_id = false, $location_id = false, $from = '', $to = '', $limit = 20, $offset = 0, $column = 'name', $orderby = 'asc', $fields = 'all') {
        $current_location = $this->Employee->get_logged_in_employee_current_location_id();
        ...
        if ($location_id) {
            $this->db->where($this->db->dbprefix('items') . '.item_location_id', $location_id);
        }
        if ($from != '' && $to != '') {
            $this->db->where($this->db->dbprefix('items') . '.create_on >= ', $from);
            $this->db->where($this->db->dbprefix('items') . '.create_on <= ', $to);
        } else {
            if ($from != '') {
                $this->db->where($this->db->dbprefix('items') . '.create_on >= ', $from);
            }
    ...
    function search_items_count_all($search, $category_id = false, $location_id = false, $from = '', $to = '', $limit = 20, $offset = 0, $column = 'name', $orderby = 'asc', $fields = 'all') {
        $current_location = $this->Employee->get_logged_in_employee_current_location_id();
    ...
    function sync_payment($item_id = 0, $item_type = 0) {
        $pending_payments = $this->Payment->get_pending_payment_approval($item_id = 0, $item_type = 0);

        if (is_object($pending_payments)) {

            foreach ($pending_payments->result() as $pending_item) {

                $item_pending_payments = $this->Payment->get_pending_payments_by_item_id($pending_item->item_id, $pending_item->item_type); // 2 for personal items

                if (isset($item_pending_payments)) {
                    foreach ($item_pending_payments->result() as $item_payment) {
                        
                        if (isset($item_payment) && is_object($item_payment)) { // at least we request a payment
                            // check how old it is. if it is old than 1 hour, then we delete it so that the system can request another payment
                            date_default_timezone_set('Africa/Cairo');
                            $t1 = time();
                            $t2 = strtotime($item_payment->date_created);
                            $diff = $t1 - $t2;
                            $payment_old = round($diff / ( 60), 0); // In minutes
//                            echo 'Date Created: '.$item_payment->date_created.'<br />';
//                            echo 'Now: '.date('Y-m-d H:i:s',$t1).'<br />';
//                            echo 'Old : '.$payment_old.' minutes<br />';;
//                            echo 'Time out: '.PAYMENT_CONFIRMATION_TIMEOUT_TIME_MINITUTES.' minutes<br />';
//                            exit();

                            if (intval($payment_old) > PAYMENT_CONFIRMATION_TIMEOUT_TIME_MINITUTES) {
                                
                                // Check if the payment has been approved
                                $this->Payment->get_request_mobile_payment_status($item_payment->transaction_id);
    ...
    function get_pending_payment_approval($item_id = 0, $item_type = 0) {
    ...
    function get_request_mobile_payment_status($transaction_id) {
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => "http://opay.rw/8081/payment/check",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
            ...
            CURLOPT_HTTPHEADER => array(
                "cache-control: no-cache",
                "content-type: application/json",
                "postman-token: 2bd377d2-829d-98b3-f4a7-4974b6002c5a"
            ),
        ));
        $response = curl_exec($curl);
        $err = curl_error($curl);
        ...
        if (!$err) {
            ...
            $request_body = $response_array['body'];
            $request_code = $request_body['id'];
            $request_status_name = $request_body['name'];
            if (count($payments)) {
                foreach ($payments->result() as $selected_payment) {
                    $selected_payment = $this->Payment->get_info($selected_payment->payment_id);
                    if (is_object($selected_payment)) {
                        
                        if ($selected_payment->payment_gatway_code != intval($request_code)) {

                            if ($request_code == REJECTED_BY_THE_PAYER) { // Payment Cancelled by the user
                                //mark the sale as deleted
                                $payment_data = array('deleted' => 1);
                                $this->Payment->save($payment_data, $selected_payment->payment_id);
                            }
                            //we now update the status
                            $payment_data = array('payment_gatway_code' => $request_code, 'payment_gatway_message' => $request_status_name);
                            $this->Payment->save($payment_data, $selected_payment->payment_id);
                            $selected_payment = $this->Payment->get_info($selected_payment->payment_id);

                            if ($request_code == SUCCESS_STATUS_CODE) { // Set the payment id to the item
                                $personal_item_data = array('owner_payment_id' => $selected_payment->payment_id);
                                if ($selected_payment->item_type == 1) {
                                    $this->Item->save($personal_item_data, $selected_payment->item_id); // Found items
                                } else {
                                    $this->Personal_item->save($personal_item_data, $selected_payment->item_id); //Personal items
                                }
                            }
                        }
                    }
