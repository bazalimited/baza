...
class Staff extends CI_Model {
    ...
    function update($staff_data, $customer_id) {
        $this->db->where('customer_id', $customer_id);
    ...
    function save($items, $supplier_id, $employee_id, $comment, $payment_type, $customer_id = false, $suspended = 0, $mode = 'receive', $change_receiving_date = false, $is_po = 0, $location_id = -1) {
        if (count($items) == 0)
            return -1;
        ...
        if ($change_receiving_date) {
            ...
            if ($receiving_time !== FALSE) {
                $staffs_data['receiving_time'] = date('Y-m-d H:i:s', strtotime($change_receiving_date));
            }
        } else {
            $staffs_data['receiving_time'] = date('Y-m-d H:i:s');
        }


        if ($customer_id) {
            $previous_receiving_items = $this->get_receiving_items($customer_id)->result_array();
            //Delete previoulsy receving so we can overwrite data
            $this->delete($customer_id, true, false);


            $this->db->where('customer_id', $customer_id);
            $this->db->update('staffs', $staffs_data);
        } else {
        ...
        foreach ($items as $line => $item) {
            ...
            $this->db->insert('staffs_items', $staffs_items_data);

            if ($suspended == 0) {
                if ($this->config->item('calculate_average_cost_price_from_staffs')) {
                    $staffs_items_data['item_unit_price_before_tax'] = $item_unit_price_before_tax;
                    $this->calculate_and_update_average_cost_price_for_item($item['item_id'], $staffs_items_data);
                    unset($staffs_items_data['item_unit_price_before_tax']);
                }
            }
            ...
            if (!$cur_item_info->is_service) {
                ...
                if ($suspended == 0 && $item['quantity_received'] === NULL) {
                    ...
                    if ($suspended == 0) {
                        $inventory_to_add = $item['quantity'] - $item['quantity_received'];
                    } else {
                        $previous_amount_received = $this->_get_quantity_received($previous_receiving_items, $item['item_id']);
                        $inventory_to_add = $item['quantity_received'] - $previous_amount_received;
                    }
                }
            ...
            if ($this->config->item('charge_tax_on_recv')) {
                foreach ($this->Item_taxes_finder->get_info($item['item_id'], 'receiving') as $row) {
                    ...
                    if (!in_array($tax_name, $this->receiving_lib->get_deleted_taxes())) {
                        $this->db->insert('staffs_items_taxes', array(
                            'customer_id' => $customer_id,
                            'item_id' => $item['item_id'],
                            'line' => $item['line'],
                            'name' => $row['name'],
    ...
    function delete($customer_id, $all_data = false, $update_quantity = true) {
        $recv_info = $this->get_info($customer_id)->row_array();
        $suspended = $recv_info['suspended'];

        $employee_id = $this->Employee->get_logged_in_employee_info()->person_id;

        if ($update_quantity) {
            $this->db->select('staffs.location_id, item_id, quantity_purchased, quantity_received, transfer_to_location_id');
            $this->db->from('staffs_items');
            $this->db->join('staffs', 'staffs.customer_id = staffs_items.customer_id');
            $this->db->where('staffs.customer_id', $customer_id);
        ...
        if ($all_data) {
            $this->db->delete('staffs_items', array('customer_id' => $customer_id));
            $this->db->delete('staffs_items_taxes', array('customer_id' => $customer_id));
        }

        $this->db->where('customer_id', $customer_id);
        return $this->db->update('staffs', array('deleted' => 1, 'deleted_by' => $employee_id));
    }
    ...
    function undelete($customer_id) {
        ...
        $this->db->where('customer_id', $customer_id);
        return $this->db->update('staffs', array('deleted' => 0, 'deleted_by' => NULL));
    }

    function get_receiving_items($customer_id) {
        $this->db->from('staffs_items');
        $this->db->where('customer_id', $customer_id);
        return $this->db->get();
    }
    ...
    public function create_staffs_items_temp_table($params) {
        set_time_limit(0);
        ...
        if (isset($params['start_date']) && isset($params['end_date'])) {
            $where = 'WHERE receiving_time BETWEEN "' . $params['start_date'] . '" and "' . $params['end_date'] . '"' . ' and ' . $this->db->dbprefix('staffs') . '.location_id IN (' . $location_ids . ')';
        ...
        $this->db->query("CREATE TEMPORARY TABLE " . $this->db->dbprefix('staffs_items_temp') . "
		(SELECT " . $this->db->dbprefix('staffs') . ".location_id as location_id, " . $this->db->dbprefix('staffs') . ".deleted as deleted," . $this->db->dbprefix('staffs') . ".deleted_by as deleted_by, receiving_time, date(receiving_time) as receiving_date, " . $this->db->dbprefix('staffs_items') . ".customer_id, comment,payment_type, employee_id, 
		" . $this->db->dbprefix('items') . ".item_id, " . $this->db->dbprefix('staffs') . ".supplier_id, quantity_purchased,quantity_received, item_cost_price, item_unit_price," . $this->db->dbprefix('categories') . ".name as category," . $this->db->dbprefix('categories') . ".id as category_id,
		...
		*(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
		...
		" . $this->db->dbprefix('staffs_items') . ".line as line, serialnumber, " . $this->db->dbprefix('staffs_items') . ".description as description
		FROM " . $this->db->dbprefix('staffs_items') . "
		INNER JOIN " . $this->db->dbprefix('staffs') . " ON  " . $this->db->dbprefix('staffs_items') . '.customer_id=' . $this->db->dbprefix('staffs') . '.customer_id' . "
		INNER JOIN " . $this->db->dbprefix('items') . " ON  " . $this->db->dbprefix('staffs_items') . '.item_id=' . $this->db->dbprefix('items') . '.item_id' . "
		LEFT OUTER JOIN " . $this->db->dbprefix('staffs_items_taxes') . " ON  "
                . $this->db->dbprefix('staffs_items') . '.customer_id=' . $this->db->dbprefix('staffs_items_taxes') . '.customer_id' . " and "
                . $this->db->dbprefix('staffs_items') . '.item_id=' . $this->db->dbprefix('staffs_items_taxes') . '.item_id' . " and "
                . $this->db->dbprefix('staffs_items') . '.line=' . $this->db->dbprefix('staffs_items_taxes') . '.line' . "
		LEFT OUTER JOIN " . $this->db->dbprefix('categories') . " ON  " . $this->db->dbprefix('categories') . '.id=' . $this->db->dbprefix('items') . '.category_id' . "
			
		$where
		GROUP BY customer_id, item_id, line)");
    }

    function drop_staffs_items_temp_table() {
        $this->db->query('DROP TABLE IF EXISTS ' . $this->db->dbprefix('staffs_items_temp'));
    }

    function calculate_and_update_average_cost_price_for_item($item_id, $current_staffs_items_data) {
        //Dont calculate averages unless we receive quanitity > 0
        if ($current_staffs_items_data['quantity_purchased'] > 0) {
            ...
            if ($averaging_method == 'moving_average') {
                ...
                $received_cost_price = $current_staffs_items_data['item_unit_price_before_tax'] * (1 - ($current_staffs_items_data['discount_percent'] / 100));
                $received_quantity = $current_staffs_items_data['quantity_purchased'];
                ...
                if ($cur_item_location_info && $cur_item_location_info->cost_price) {
					...
					FROM " . $this->db->dbprefix('staffs_items') . ' ' .
                                    'JOIN ' . $this->db->dbprefix('staffs') . ' ON ' . $this->db->dbprefix('staffs') . '.customer_id = ' . $this->db->dbprefix('staffs_items') . '.customer_id ' .
                ...
                $cost_price_avg = $current_staffs_items_data['item_unit_price_before_tax'];
            }
    ...
    function get_all_suspended() {
        ...
        $this->db->from('staffs');
        $this->db->join('suppliers', 'staffs.supplier_id = suppliers.person_id', 'left');
        $this->db->join('people', 'suppliers.person_id = people.person_id', 'left');
        $this->db->where('staffs.deleted', 0);
        $this->db->where('staffs.suspended', 1);
        $this->db->where('location_id', $location_id);
        $this->db->order_by('customer_id');
        $staffs = $this->db->get()->result_array();

        for ($k = 0; $k < count($staffs); $k++) {
            $item_names = array();
            $this->db->select('name');
            $this->db->from('items');
            $this->db->join('staffs_items', 'staffs_items.item_id = items.item_id');
            $this->db->where('customer_id', $staffs[$k]['customer_id']);

            foreach ($this->db->get()->result_array() as $row) {
                $item_names[] = $row['name'];
            }

            $staffs[$k]['items'] = implode(', ', $item_names);
        }

        return $staffs;
    }

    function get_suspended_staffs_for_item($item_id) {
        $this->db->from('staffs');
        $this->db->join('staffs_items', 'staffs.customer_id = staffs_items.customer_id');
        $this->db->where('staffs.suspended', '1');
        $this->db->where('staffs_items.item_id', $item_id);

        return $this->db->get()->result_array();
    }

    function get_receiving_items_taxes($customer_id, $line = FALSE) {
        $item_where = '';

        if ($line) {
            $item_where = 'and ' . $this->db->dbprefix('staffs_items') . '.line = ' . $line;
        }
        ...
        $query = $this->db->query('SELECT name, percent, cumulative, item_unit_price as price, quantity_purchased as quantity, discount_percent as discount ' .
                'FROM ' . $this->db->dbprefix('staffs_items_taxes') . ' JOIN ' .
                $this->db->dbprefix('staffs_items') . ' USING (customer_id, item_id, line) ' .
                'WHERE ' . $this->db->dbprefix('staffs_items_taxes') . ".customer_id = $customer_id" . ' ' . $item_where . ' ' .
                'ORDER BY ' . $this->db->dbprefix('staffs_items') . '.line,' . $this->db->dbprefix('staffs_items') . '.item_id,cumulative,name,percent');
        return $query->result_array();
    }

    function get_deleted_taxes($customer_id) {
        $this->db->from('staffs');
        $this->db->where('customer_id', $customer_id);
