...
class Sale extends CI_Model {
            ...
            if ($this->config->item('loyalty_option') == 'simple') {
                if (!$store_account_payment) {
                    if ($this->sale_lib->get_redeem()) {
                        ...
                        $this->db->update('customers');
                        $sales_data['did_redeem_discount'] = 1;
                    } else {
                        $this->db->where('person_id', $customer_id);
                ...
                if (!$store_account_payment) {
                    ...
                    if ($current_spend_for_points + $total_spend_for_sale >= $spend_amount_for_points) {
                        $new_point_value = $current_points + (((($current_spend_for_points + $total_spend_for_sale) - (($current_spend_for_points + $total_spend_for_sale) % $spend_amount_for_points)) / $spend_amount_for_points) * $points_to_earn);
                        $new_current_spend_for_points = (($current_spend_for_points + $total_spend_for_sale) % $spend_amount_for_points);
                    } else {
                        $new_current_spend_for_points = $current_spend_for_points + $total_spend_for_sale;
                        $new_point_value = $current_points;
                    }

                    $sales_data['points_gained'] = $new_point_value - $current_points;
                ...
                if ($payment_amount_points = $this->sale_lib->get_payment_amount(lang('common_points'))) {
                    ...
                    $sales_data['points_used'] = $points_used;
                } else {
                    $sales_data['points_used'] = 0;
                }
    ...
    function update_points($sale_id, $undelete = 0) {
        ...
        if (!$store_account_payment && $suspended != 2 && $customer_id > 0 && $this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'advanced') {
            ...
            if ($undelete) { //Put points back
                ...
                $new_point_value = to_currency_no_money($new_point_value);
                $new_current_spend_for_points = to_currency_no_money($new_current_spend_for_points);
                ...
                if ($current_spend_for_points - $total_spend_for_sale >= 0) { //Just need to remove current spend
                    $new_point_value = $current_points;
                    $new_current_spend_for_points = $current_spend_for_points - $total_spend_for_sale;
                } else {
    ...
    function get_sale_total($sale_id) {
        $this->Sale->create_sales_items_temp_table(array('sale_ids' => array($sale_id)));
    ...
    function _create_sales_items_temp_table_query($where) {
		...
		*(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
		ROUND((item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)," . $decimals . ") - (item_cost_price*quantity_purchased) as profit, commission, store_account_payment,item_cost_price as sale_item_temp_cost_price, points_used, points_gained
		...
		discount_percent, ROUND(item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100," . $decimals . ") as subtotal,
		...
		(ROUND(item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100," . $decimals . "))+(item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) 
		...
		*(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as total,
		...
		+(((item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) + (item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100))
		*(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
		ROUND((item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)," . $decimals . ") - (item_kit_cost_price*quantity_purchased) as profit, commission, store_account_payment, item_kit_cost_price as sale_item_temp_cost_price, points_used, points_gained
