...
class Item extends CI_Model {
    ...
    function get_all_by_user($user_id, $limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc') {
		...
		categories.id as category_id, phppos_locations.name as locationa_name');
    ...
    function get_all_unpaid_by_user($user_id, $limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc', $agent_not_paid = true, $owner_not_paid = true) {
        ...
        $this->db->where('(items.owner_payment_id is null or items.agent_payment_id is null)');
    ...
    function get_all_uncollected_by_user($user_id, $location_id = null, $limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc') {
        ...
        $this->db->where('(items.owner_payment_id is not null)');
    ...
    function count_all_uncollected_by_user($user_id, $location_id, $col = 'item_id', $order = 'desc', $agent_not_paid = true, $owner_not_paid = true) {
    ...
    function get_all_unpaid_agents_by_user($user_id, $location_id = null, $limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc') {
        ...
        $this->db->join('phppos_locations', 'phppos_items.item_location_id = phppos_locations.location_id', 'left');
        $this->db->where('items.deleted', 0);
        $this->db->where('items.status', 1); //Must be Collected
        $this->db->where('(items.owner_payment_id is not null)');
    ...
    function count_unpaid_agents_by_user($user_id, $location_id = null, $limit = 10000, $offset = 0, $col = 'item_id', $order = 'desc') {
    ...
    function found_item_paid($item_id) {
        $result = $this->db->query("CALL check_unpaid_by_found_item(" . $item_id . ", '" . SUCCESS_STATUS_CODE . ", " . PENDING_STATUS_CODE . "," . PENDING_COMMIT_TO_WALLET_STATUS_CODE . "')");
    ...
    function item_payment_status($item_id) {
        $item_payment_status = $this->Payment->get_payment_by_item_id($item_id, 1); // 1 for found items
    ...
    function is_collected($item_id) {
        ...
        if ($query->num_rows() == 1) {
            $item = $query->row();
            if ($item->status == 1) { // 1 is for collected items
                return true;
            }
        }
        return false;
    ...
    function owner_not_payed($item_id) {
        $this->db->from('items');
        $this->db->where('item_id', $item_id);
    ...
    function agent_not_payed($item_id) {
        $this->db->from('items');
        $this->db->where('item_id', $item_id);
    ...
    function search_items($search, $category_id = false, $location_id = false, $from = '', $to = '', $limit = 20, $offset = 0, $column = 'name', $orderby = 'asc') {

        $locations_list = $this->Employee->get_employee_locations();
        $location_ids = array();
        if (count($locations_list) > 0) {
            foreach ($locations_list as $key => $value) {
                array_push($location_ids, $key);
        ...
        $this->db->where("(" . $this->db->dbprefix('items') . ".name like '%" . escape_full_text_boolean_search($search) . "%' or " . $this->db->dbprefix('items') . ".item_number like '%" . escape_full_text_boolean_search($search) . "%' or description like '%" . escape_full_text_boolean_search($search) . "%' )");
    ...
    function search_uncollected($search, $category_id = false, $location_id = false, $from = '', $to = '', $limit = 20, $offset = 0, $column = 'name', $orderby = 'asc', $fields = 'all') {
    ...
    function public_search_items($search) {
        ...
        $this->db->from('items');
        $this->db->join('categories', 'categories.id = items.category_id', 'left');
        $this->db->where('(' . $this->db->dbprefix('items') . ".name like '%" . $search . "%' or " . $this->db->dbprefix('items') . ".item_number like '%" . $search . "%' or  product_id like '%" . $search . "%' or description like '%" . $search . "%') and " . $this->db->dbprefix('items') . ".deleted=0");
        $this->db->where('items.owner_payment_id is null');
        $this->db->where('items.status = 0'); //Not Collected
    ...
    function public_search_items_pictures($category_id = '0') {
        $this->db->select('items.*,categories.name as category');
        $this->db->from('items');
        $this->db->join('categories', 'categories.id = items.category_id', 'left');
        $this->db->where('items.owner_payment_id is null and image_id is not null');
        $this->db->where('items.status = 0'); //Not Collected
    ...
    function public_search_items_pictures_count($category_id = '0') {
        $this->db->select('items.*,categories.name as category');
        $this->db->from('items');
        $this->db->join('categories', 'categories.id = items.category_id', 'left');
        $this->db->where('items.owner_payment_id is null and image_id is not null');
    ...
    function get_categories() {
        ...
        $user_items = $this->Item->public_search_items_pictures();
        $category_ids = array();
    ...
    function search_uncollected_items_count_all($search, $category_id = false, $location_id = false, $from = '', $to = '', $limit = 20, $offset = 0, $column = 'name', $orderby = 'asc', $fields = 'all') {
        $locations_list = $this->Employee->get_employee_locations();
        $location_ids = array();
        if (count($locations_list) > 0) {
            foreach ($locations_list as $key => $value) {
    ...
    function get_found_item_status($item_id) {
        ...
        if (!isset($item_payment)) {
            ...
            if (in_array($item_payment->payment_gatway_code, array(PENDING_STATUS_CODE, PENDING_COMMIT_TO_WALLET_STATUS_CODE))) {
                return ITEM_STATUS_PAYMENT_PENDIND;
            } else if ($item_payment->payment_gatway_code == SUCCESS_STATUS_CODE) {
                if ($item->status == 0) { //Not collected
                    return ITEM_STATUS_PAYMENT_CONFIRMED_PENDING_DELIVERY;
                } else {
                    return ITEM_STATUS_PAYMENT_CONFIRMED_DELIVERY_CONFIRMED;
                }
            } else {
                return ITEM_STATUS_FOUND;
            }
        }
        return ITEM_STATUS_FOUND;
    }
