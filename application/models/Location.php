...
class Location extends CI_Model {
    ...
    function get_search_suggestions($search, $limit = 25) {
        ...
        if ($this->config->item('supports_full_text') && !$this->config->item('legacy_search_method')) {
            ...
            $this->db->select("location_id, address, MATCH (address) AGAINST (" . $this->db->escape(escape_full_text_boolean_search($search) . '*') . " IN BOOLEAN MODE) as rel", false);
            $this->db->from('locations');
            $this->db->where('deleted', 0);
    ...
    function assign_employees_to_location($location_id, $employees) {
        ...
        foreach ($employees as $employee_id) {
            $this->db->insert('employees_locations', array(
