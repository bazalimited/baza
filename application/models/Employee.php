...
class Employee extends Person {
    ...
    function is_not_employee($id) {
        ...
        if ($query->num_rows() == 1) {
            if ($query->row()->is_employee == 0) { // 0 for non Employees. Online Users
                return true;
            } else {
                return false;
            }
    ...
    function employee_phone_exists($phone) {
        $this->db->from('employees');
        $this->db->join('people', 'people.person_id = employees.person_id');
    ...
    function get_person_id_from_employee_id($employee_id, $can_cache = TRUE) {
        if ($can_cache) {
            static $cache = array();
                ...
                if ($success) {
                    if ($location_data !== FALSE) {
                        foreach ($location_data as $location_id) {
                            $success = $this->db->insert('employees_locations', array(
    ...
    function get_employee_locations($employee_id = null) {
        if (isset($employee_id)) {
            $logged_in_employee_info = $this->Employee->get_info($employee_id);
        } else {
    ...
    function get_employee_locations_selection($employee_id = null) {
        if (isset($employee_id)) {
            $logged_in_employee_info = $this->Employee->get_info($employee_id);
        } else {
            $logged_in_employee_info = $this->Employee->get_logged_in_employee_info();
        ...
        $authenticated_locations = $this->Employee->get_authenticated_location_ids($logged_in_employee_info->person_id);
        $locations = array();
        $locations[''] = lang('common_location_all');
        foreach ($locations_list->result() as $row) {
            if (in_array($row->location_id, $authenticated_locations)) {
                $locations[$row->location_id] = $row->name;
            }
    ...
    function get_total_employee_locations($employee_id = null) {
        if (isset($employee_id)) {
            $logged_in_employee_info = $this->Employee->get_info($employee_id);
        } else {
            $logged_in_employee_info = $this->Employee->get_logged_in_employee_info();
        ...
        return $total_locations_in_system;
    }
