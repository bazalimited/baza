...
//new line endings
    ...
    function index($offset = 0) {
        ...
        $config['total_rows'] = $this->Item->count_unpaid_agents_by_user($employee_id, $current_location_id, $data['per_page'], $params['offset'], $params['order_col'], $params['order_dir']);
        $table_data = $this->Item->get_all_unpaid_agents_by_user($employee_id, $current_location_id, $data['per_page'], $params['offset'], $params['order_col'], $params['order_dir']);
        ...
        $data['manage_table'] = get_unpaid_agents_manage_table($table_data, $this);

        $this->load->view('payments/manage', $data);
    }

    function item_collection($offset = 0) {
        ...
        $data['manage_table'] = get_uncollected_items_manage_table($table_data, $this);

        $this->load->view('payments/item_collection', $data);
    }
    ...
    function pay_agent() {
        ...
        $payment_type = 1; // Agent Payment
        $payment_method_id = 1; // Manual payment
        $amount = $this->input->post('agent_amount');
        $payment_date = ($this->input->post('payment_date') == '') ? '' : date("Y-m-d", strtotime($this->input->post('payment_date')));
        $payed_by_id = $this->Employee->get_logged_in_employee_info()->person_id;

        $payment_data = array(
            'item_id' => $item_id,
            'item_type' => 1, // 1 for found items
            'agent_id' => $agent_id,
        ...
        if ($payment_id > 0) {
//update the item satatus
            $item_data = array();
            $item_data['agent_payment_id'] = $payment_id;
    ...
    function register_owner_payment() {
        $item_id = $this->input->post('item_id');
        $agent_id = $this->input->post('agent_id');
        $payment_type = 2;
        $payment_method_id = 1;
...
//update the item satatus
            $item_data = array();
            $item_data['owner_payment_id'] = $payment_id;
            $this->Item->save($item_data, $item_id);
            redirect('payments');
        } else {
            echo 'Save Failed';
    ...
    function save_category($category_id = FALSE) {
        ...
        if ($this->Category->save($category_name, $hide_from_grid, $parent_id, $category_id, $category_location_id)) {
        ...
        if (!is_null($tree) && count($tree) > 0) {
            ...
            foreach ($tree as $node) {
                $return .= '
                        <tr>
                            <td>' . $node->name . '</td>
                            <td><a href="javascript:void(0);" class="edit_category" data-name = "' . H($node->name) . '" data-parent_id = "' . $node->parent_id . '" data-category_id="' . $node->id . '">[' . lang('common_edit') . ']</a> </td>
    ...
    function search_uncollected() {
        ...
        $this->session->set_userdata("item_search_data", $item_search_data);
        $per_page = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $search_data = $this->Item->search_uncollected($search, $category_id, $location_id, $from_date, $to_date, $per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'name', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'asc');

        $config['base_url'] = site_url('payments/search_uncollected');

        $config['total_rows'] = $this->Item->search_uncollected_items_count_all($search, $category_id, $location_id, $from_date, $to_date, $per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'name', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'asc', $fields);
        ...
        $data['manage_table'] = get_uncollected_items_table($search_data, $this);
...
//allow parallel searchs to improve performance.
    ...
    function return_item($item_id = -1) {

        $this->check_action_permission('approve_item_return');
        $this->Item_collection->approve_return($item_id);
        redirect($_SERVER['HTTP_REFERER']);
    }

    function collect($item_id = -1, $supplier_id = -1, $redirect = 0, $sale_or_receiving = 'sale') {
    ...
    function save_collect() {
        //Save Image File
        if (!empty($_FILES["consent_id"]) && $_FILES["consent_id"]["error"] == UPLOAD_ERR_OK && $_FILES["consent_id"]["size"] < 1048576) {
            $allowed_extensions = array('png', 'jpg', 'pdf', 'jpeg');
            $extension = strtolower(pathinfo($_FILES["consent_id"]["name"], PATHINFO_EXTENSION));
            if (in_array($extension, $allowed_extensions)) {

                $this->load->model('Appfile');
                $image_file_id = $this->Appfile->save($_FILES["consent_id"]["name"], file_get_contents($_FILES["consent_id"]["tmp_name"]));
            } else {
                $message = '<div class="alert alert-danger"><strong>Only pdf, png, jpg, and jpeg files are allowed</strong></div>';
                $this->session->set_userdata("save_collect_message", $message);
                redirect('payments/collect/' . $this->input->post('item_id'));
            }
            ...
            if ($image_file_id > 0) {
                $item_collection_data = array(
                    ...
                    'collection_comment' => $this->input->post('collection_comment'),
                    'collection_date' => ($this->input->post('collection_date') == '') ? '' : date("Y-m-d", strtotime($this->input->post('collection_date'))),
                    'consent_form_id' => $image_file_id,
                    'created_by' => $this->Employee->get_logged_in_employee_info()->person_id,
                );
                $collect_id = $this->Item_collection->save($item_collection_data);
                if ($collect_id > 0) {
                    //set item status to collected
                    $item_data = array('status' => 1);
                    $this->Item->save($item_data, $this->input->post('item_id'));
                    redirect('payments/collect/' . $this->input->post('item_id'));
                }
            } else {
                $message = '<div class="alert alert-danger"><strong>Fail to save the consent form</strong></div>';
                $this->session->set_userdata("save_collect_message", $message);
                redirect('payments/collect/' . $this->input->post('item_id'));
            }
        } else {
            $message = '<div class="alert alert-danger"><strong>Fail to save the consent form. Maximum allowed size is 1 MB</strong></div>';
            $this->session->set_userdata("save_collect_message", $message);
            redirect('payments/collect/' . $this->input->post('item_id'));
        }
    }
