...
class Reports extends Secure_area {
                ...
                if ($this->input->get('term') != '') { // What exactly are we searchin
                    ...
                    switch ($this->input->get('w')) {
                        ...
                        case 'itemsCategory':
                            ...
                            $tmp = array();
                            foreach ($t as $k => $v) {
                                $tmp[$k] = array('id' => $v['id'], 'name' => $v['label']);
                            }
                            die(json_encode($tmp));
                            break;
                        case 'suppliers':
                        ...
                        case 'itemsName':
                            ...
                            $tmp = array();
                            foreach ($t as $v) {
                                $tmp[] = array('id' => $v['value'], 'name' => $v['label']);
                            }
                            die(json_encode($tmp));
                            break;
                        case 'paymentType':
            ...
            foreach ($field as $a => $b) {
                ...
                switch ($b) {
                    ...
                    case '4': // Items Category
                        ...
                        foreach ($t as $k => $v) {
                            $tmpID[] = $v->id;
                            $tmp[$k] = array('id' => $v->id, 'name' => $v->name);
                        }
                        break;
                    case '5': // Suppliers 
                        $this->load->model('Supplier');
    ...
    /** added for register log */
    function revenuep_date_input_no_revenue() {
        $data = $this->_get_common_report_data();
        $locations = array();
        foreach ($this->Location->get_all()->result() as $location_row) {
            $locations[$location_row->location_id] = $location_row->name;
        ...
        $this->load->view("reports/revenuep_date_input_no_revenue", $data);
    }
    ...
    function detailed_register_log($start_date, $end_date, $export_excel = 0, $offset = 0) {
        ...
        foreach ($report_data as $row) {
            if ($row['shift_end'] == '0000-00-00 00:00:00') {
                ...
                $delete = anchor('reports/delete_register_log/' . $row['register_log_id'] . '/' . $start_date . '/' . $end_date, lang('common_delete'), "onclick='return do_link_confirm(" . json_encode(lang('reports_confirm_register_log_delete')) . ", this)'");
            } else {
    ...
    function summary_items($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $supplier_id = -1, $category_id = -1, $sale_type = 'all', $export_excel = 0, $offset = 0) {
        ...
        $config['total_rows'] = $model->getItemsDataTotalRows();
        ...
        $tabular_data = array();
        $report_data = $model->getItemsData();

        $counter = 1;
        foreach ($report_data as $row) {
            $data_row = array();
            $data_row[] = array('data' => $counter, 'align' => 'left');
            $data_row[] = array('data' => $row['name'], 'align' => 'left');
            if (strlen($row['item_number']) >= 2) { // 2 is Just a Random String
                $data_row[] = array('data' => $row['item_number'], 'align' => 'left');
            } else {
                $data_row[] = array('data' => $row['name_on_card'], 'align' => 'left'); // Name on the card
            }
            $data_row[] = array('data' => $row['ctegory_name'], 'align' => 'left');
            $data_row[] = array('data' => $row['location_name'], 'align' => 'left');
            $data_row[] = array('data' => date(get_date_format(), strtotime($row['create_on'])), 'align' => 'left');
            $tabular_data[] = $data_row;
            $counter++;
        }
        ...
        $data = array(
            "title" => lang('reports_items_found_items'),
            "subtitle" => date(get_date_format(), strtotime($start_date)) . '-' . date(get_date_format(), strtotime($end_date)) . ($do_compare ? ' ' . lang('reports_compare_to') . ' ' . date(get_date_format(), strtotime($compare_start_date)) . '-' . date(get_date_format(), strtotime($compare_end_date)) : ''),
            "headers" => $model->getReportDataColumns(),
            "data" => $tabular_data,
        ...
        $this->load->view("reports/tabular_items_by_location", $data);
    }
    ...
    function store_account_statements_email_customer($customer_id, $start_date, $end_date, $hide_items = 0, $pull_payments_by = 'payment_date', $offset = 0) {
        ...
        if (!empty($customer_info->email)) {
            ...
            $this->email->from($this->Location->get_info_for_key('email') ? $this->Location->get_info_for_key('email') : 'no-reply@mg.baza.rw', $this->config->item('company'));
    ...
    function detailed_inventory($start_date, $end_date, $item_id, $show_manual_adjustments_only, $export_excel = 0, $offset = 0) {
        ...
        foreach ($report_data as $row) {
            ...
            $tabular_data_row = array(
                ...
                array('data' => $this->Customer->get_customer_name($row['customer_id']), 'align' => 'left'),
            );
    ...
    function detailed_timeclock($start_date, $end_date, $employee_id, $export_excel = 0, $offset = 0) {
        ...
        foreach ($report_data as $row) {
            ...
            $delete = anchor('timeclocks/delete/' . $row['id'] . '/' . $start_date . '/' . $end_date . '/' . $employee_id, lang('common_delete'), "onclick='return do_link_confirm(" . json_encode(lang('reports_confirm_timeclock_delete')) . ", this)'");
    ...
    function closeout($date, $export_excel = 0) {
        ...
        $model = $this->Closeout;
        $model->setParams(array('date' => $date, 'export_excel' => $export_excel));
    ...
    function expiring_inventory($start_date, $end_date, $location_id, $export_excel = 0, $offset = 0) {
        $start_date = rawurldecode($start_date);
        $end_date = rawurldecode($end_date);
        ...
        $model->setParams(array('start_date' => $start_date, 'end_date' => $end_date, 'export_excel' => $export_excel, 'offset' => $offset, 'location_id' => $location_id));

        $config = array();
        $config['base_url'] = site_url("reports/expiring_inventory/$start_date/$end_date/$location_id/$export_excel");
        $config['total_rows'] = $model->getTotalRows();
        $config['per_page'] = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $config['uri_segment'] = 7;
        $this->load->library('pagination');
    ...
    //Detailed expenses report
    function revenuep($start_date, $end_date, $export_excel = 0, $offset = 0) {
        $this->check_action_permission('view_expenses');
        $start_date = rawurldecode($start_date);
        $end_date = rawurldecode($end_date);

        $this->check_action_permission('view_internal');
        $this->load->model('reports/Detailed_payment');

        $model = $this->Detailed_payment;
        $model->setParams(array('start_date' => $start_date, 'end_date' => $end_date, 'offset' => $offset, 'export_excel' => $export_excel));
        $config = array();
        $config['base_url'] = site_url("reports/revenuep/" . rawurlencode($start_date) . '/' . rawurlencode($end_date) . "/$export_excel");
        ...
        foreach ($report_data as $row) {
            $tabular_data_row = array(
                ...
                array('data' => $row['expense_description'], 'align' => 'left'),
                array('data' => $row['category_name'], 'align' => 'left'),
                array('data' => date(get_date_format(), strtotime($row['payment_date'])), 'align' => 'left'),
                array('data' => to_currency($row['amount']), 'align' => 'left')
            );
            if ($row['owner_payment_id'] != 0) {
                $total_owner_payment += $row['amount'];
            }
            if ($row['agent_payment_id'] != 0) {
                $total_agent_payment += $row['amount'];
            }
        ...
        $total_revenue = $total_owner_payment - $total_agent_payment;
        $data = array(
            "title" => lang('revenue_by_period_detailed_report'),
            "subtitle" => date(get_date_format(), strtotime($start_date)) . '-' . date(get_date_format(), strtotime($end_date)),
            "headers" => $model->getDataColumns(),
            "data" => $tabular_data,
            "summary_data" => $model->getSummaryData(),
            ...
            "total_agent_payment" => $total_agent_payment,
            "total_revenue" => $total_revenue,
        );

        $this->load->view("reports/revenue_by_date", $data);
    }
