...
class Sales extends Secure_area {
    ...
    function item_search() {
        ...
        session_write_close();
        $suggestions = $this->Item->get_item_search_suggestions($this->input->get('term'), 100, $this->Employee->get_logged_in_employee_current_location_id());
    ...
    function add_payment() {
        ...
        if ($payment_type == lang('common_points')) {
            ...
            if ($this->input->post('amount_tendered') > to_currency_no_money($customer_info->points)) {
                $data['error'] = lang('sales_points_to_much');
                $this->_reload($data);
                return;
            }

            $payment_amount = min(to_currency_no_money($this->input->post('amount_tendered') * $this->config->item('point_value')), $this->sale_lib->get_amount_due());
    ...
    function complete() {
        ...
        if ($this->sale_lib->get_customer() == -1) {
            $this->_reload(array('error' => lang('customer_required')), false);
            return;
        }
...
//        echo 'Response:<br />';
//        try {
//            $output = shell_exec('java -jar EBM.jar 101052 10000 36');
//            echo $output;
//        } catch (Exception $ex) {
//            echo $ex->getMessage();
//        }
...
//        exit();
    ...
    function categories_and_items($category_id = NULL, $offset = 0) {
        ...
        $items_result = $this->Item->get_all_by_category_and_location($category_id, $this->config->item('hide_out_of_stock_grid') ? TRUE : FALSE, $items_offset, $this->config->item('number_of_items_in_grid') ? $this->config->item('number_of_items_in_grid') : 14)->result();
    ...
    function categories($parent_id = NULL, $offset = 0) {
        ...
        $location_id = $this->Employee->get_logged_in_employee_current_location_id();
        $categories = $this->Category->get_all($parent_id, FALSE, $this->config->item('number_of_items_in_grid') ? $this->config->item('number_of_items_in_grid') : 14, $offset, 'name', 'asc', $location_id);
