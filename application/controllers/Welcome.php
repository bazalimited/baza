...
class Welcome extends MY_Controller {
    ...
    function search($search = null, $type = 'registered', $category_id = '0') {
        if (!isset($search)) {
            redirect('welcome');
        }
        $data = array();
        ...
        if ($type == 'registered') {
            $personal_search_data = $this->Personal_item->public_search_items($search);
            $data['personal_total_rows'] = $this->Personal_item->public_search_items_count($search);
            $data['personal_items_result'] = $personal_search_data->result();
        } else if ($type == 'found') {
            $search_data = $this->Item->public_search_items($search);
            $data['total_rows'] = $this->Item->public_search_items_count_all($search);
            $data['items_result'] = $search_data->result();
        } else if ($type == 'picture') {
            $search_data = $this->Item->public_search_items_pictures($category_id);
            $data['total_rows'] = $this->Item->public_search_items_pictures_count($category_id);
            $data['items_result'] = $search_data->result();
            $data['category_id'] = $category_id;
        } else {
            //
        }
        $data['categories'] = $this->Item->get_categories();
        $data['search'] = $search;
        $data['type'] = $type;
        $this->load->view("welcome/result", $data);
    }

    function item_details($item_id = null) {
        ...
        $this->load->view("welcome/item_details", $data);
    }
            ...
            if ($this->input->post('username_or_account_number') || $this->input->post('username')) {
                if ($this->input->post('username_or_account_number')) {
                    if (!$this->Employee->login_no_password($this->input->post('username_or_account_number'))) {
                        ...
                        if (!$this->config->item('keep_same_location_after_switching_employee')) {
    ...
    function do_reset_password_notify() {
        if ($this->input->post('username_or_email')) {
            ...
            if ($employee) {
                ...
                $this->email->from('no-reply@mg.baza.rw', $this->config->item('company'));
    ...
    function generate_reset_key($employee_id) {
        ...
        if ($this->db->insert('employees_reset_password', array(
