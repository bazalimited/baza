        ...
        <div class="panel panel-piluku">
            <div class="panel-body panel-pad">
                            ...
                            <ul class="list-unstyled invoice-address invoiceto">
                                ...
                                <?php if (!$this->config->item('remove_customer_contact_info_from_receipt')) { ?>
                                    ...
                                    if (!empty($customer_city)) {
                                        echo '<li>' . $customer_city . ' ' . $customer_state . ', ' . $customer_zip . '</li>';
                                    }
                                    ?>
                                    <?php
                                    if (!empty($customer_country)) {
                                        echo '<li>' . $customer_country . '</li>';
                                    }
                                    ?>			
                ...
                <div class="invoice-table">
                    <div class="row">
                        <div class="col-md-6  col-sm-6 col-xs-6 little-padding-print">
                            <div class="invoice-head"><?php echo lang('common_item_name'); ?></div>
                        </div>

                        <div class="col-md-6 col-sm-6 col-xs-6 little-padding-print">
                            <div class="invoice-head" style="float:right"><?php echo lang('common_quantity'); ?></div>
                        </div>
                    </div>
                </div>
                <?php
                ...
                foreach (array_reverse($cart, true) as $line => $item) {
                    ...
                    <div class="invoice-table-content">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6 little-padding-print">
                                <div class="invoice-content" style="float:right"><?php echo to_quantity($item['quantity']); ?></div>
                            </div>
...
<?php
if ($this->config->item('print_after_sale') && $this->uri->segment(2) == 'complete') {
    ?>
...
?>
    ...
    function SigImageCallback(str)
    {
        ...
        $.post('<?php echo site_url('sales/sig_save'); ?>', {sale_id: <?php echo $sale_id_raw; ?>, image: str}, function (response)
        {
...
<script type="text/print" id="print_output"><?php echo $this->config->item('company'); ?>
    ...
    <?php echo lang('common_item'); ?>            <?php echo lang('common_price'); ?> <?php echo lang('common_quantity'); ?><?php
    if ($discount_exists) {
        echo ' ' . lang('common_discount_percent');
    }
    ...
    foreach (array_reverse($cart, true) as $line => $item) {
        ?>
        <?php echo character_limiter($item['name'], 14, '...'); ?><?php echo strlen($item['name']) < 14 ? str_repeat(' ', 14 - strlen($item['name'])) : ''; ?> <?php echo str_replace('<span style="white-space:nowrap;">-</span>', '-', to_currency($item['price'])); ?> <?php echo to_quantity($item['quantity']); ?><?php
        if ($discount_exists) {
            echo ' ' . $item['discount'];
        }
        ?> <?php echo str_replace('<span style="white-space:nowrap;">-</span>', '-', to_currency($item['price'] * $item['quantity'] - $item['price'] * $item['quantity'] * $item['discount'] / 100)); ?>
    ...
    foreach ($payments as $payment_id => $payment) {
        ...
        <?php echo (isset($show_payment_times) && $show_payment_times) ? date(get_date_format() . ' ' . get_time_format(), strtotime($payment['payment_date'])) : lang('common_payment'); ?>  <?php
        if (($is_integrated_credit_sale || sale_has_partial_credit_card_payment()) && ($payment['payment_type'] == lang('common_credit') || $payment['payment_type'] == lang('sales_partial_credit'))) {
            ...
            echo $splitpayment[0];
            ?> <?php } ?><?php echo $this->config->item('round_cash_on_sales') && $payment['payment_type'] == lang('common_cash') ? str_replace('<span style="white-space:nowrap;">-</span>', '-', to_currency(round_to_nearest_05($payment['payment_amount']))) : str_replace('<span style="white-space:nowrap;">-</span>', '-', to_currency($payment['payment_amount'])); ?>
        <?php if ($payment['entry_method']) { ?><?php echo lang('sales_entry_method') . ': ' . $payment['entry_method']; ?><?php } ?>

        <?php if ($payment['tran_type']) { ?><?php echo lang('sales_transaction_type') . ': ' . $payment['tran_type']; ?><?php } ?>

        <?php if ($payment['application_label']) { ?><?php echo lang('sales_application_label') . ': ' . $payment['application_label']; ?><?php } ?>

        <?php if ($payment['ref_no']) { ?><?php echo lang('sales_ref_no') . ': ' . $payment['ref_no']; ?><?php } ?>

        <?php if ($payment['auth_code']) { ?><?php echo lang('sales_auth_code') . ': ' . $payment['auth_code']; ?><?php } ?>

        <?php if ($payment['aid']) { ?><?php echo 'AID: ' . $payment['aid']; ?><?php } ?>

        <?php if ($payment['tvr']) { ?><?php echo 'TVR: ' . $payment['tvr']; ?><?php } ?>

        <?php if ($payment['tsi']) { ?><?php echo 'TSI: ' . $payment['tsi']; ?><?php } ?>

        <?php if ($payment['arc']) { ?><?php echo 'ARC: ' . $payment['arc']; ?><?php } ?>

        <?php if ($payment['cvm']) { ?><?php echo 'CVM: ' . $payment['cvm']; ?><?php } ?>
        <?php
    }
    ?>	
