...
<?php if ($mode != 'store_account_payment') { ?>					
                    <table id="register" class="table table-hover">
                        ...
                        <thead>
                            <tr class="register-items-header">
                                ...
                                <th class="sales_price" style="display: none"><?php echo lang('common_price'); ?></th>
                                <th class="sales_quantity"><?php echo lang('common_quantity'); ?></th>
                                <th class="sales_discount" style="display: none"><?php echo lang('common_discount_percent'); ?></th>
                                <th style="display: none"><?php echo lang('common_total'); ?></th>
                            </tr>
                        </thead>
    ...
    <?php if (count($cart) == 0) { ?>
            ...
            <?php } ?>
                                        ...
                                        <td class="text-center" style="display: none"><?php echo to_currency($item['price'] * $item['quantity'] - $item['price'] * $item['quantity'] * $item['discount'] / 100); ?></td>
                                    </tr>
...
<?php if ($mode != 'store_account_payment') { ?>
    <?php if ($this->Employee->has_module_action_permission('giftcards', 'add_update', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
        ...
        <?php
        echo
        anchor("sales/new_giftcard", '<i class="ion-card"></i> ' . lang('sales_new_giftcard'), array('class' => '',
        ...
        ?>
                        ...
                        <li>
                            <?php
                            echo
                            anchor("sales/suspended", '<i class="ion-ios-list-outline"></i> ' . lang('sales_suspended_sales'), array('class' => '',
                            ...
                            <li>
                                <?php
                                echo
                                anchor("reports/sales_generator", '<i class="ion-search"></i> ' . lang('sales_search_reports'), array('class' => '',
                        ...
                        <?php } ?>
                            ...
                            <li>
                                <?php
                                echo anchor("sales/change_mode/store_account_payment/1", '<i class="ion-toggle-filled"></i> ' . lang('sales_store_account_payment'), array('class' => '', 'title' => lang('sales_store_account_payment')));
                                ?>
                            </li>		
                        ...
                        <li>
    <?php
    echo anchor("sales/batch_sale/", '<i class="ion-bag"></i> ' . lang('batch_sale'), array('class' => 'none suspended_sales_btn', 'title' => lang('batch_sale')));
    ?>
                        </li>					
                        <?php } ?>
...
<?php echo '<a href="#look-up-receipt" class="look-up-receipt" data-toggle="modal"><i class="ion-document"></i> ' . lang('lookup_receipt') . '</a>'; ?>
                        ...
                        if ($last_sale_id = $this->Sale->get_last_sale_id()) {
                            echo '<li>';
                            echo anchor("sales/receipt/$last_sale_id", '<i class="ion-document"></i> ' . lang('sales_last_sale_receipt'), array('target' => '_blank', 'class' => 'look-up-receipt', 'title' => lang('lookup_receipt')));
...
<?php }; ?>
        ...
        <div class="amount-block" style="display: none;">
...
<?php if (count($cart) > 0) { ?> 
            <!-- Payment Applied -->
                ...
                <ul class="list-group payments" style="display: none;">
    ...
    <?php } ?>
            ...
            <?php if ($customer_required_check) { ?>
                <div class="add-payment">
                    <div class="side-heading" style="display:none;"><?php echo lang('common_add_payment'); ?></div>
        ...
        foreach ($payment_options as $key => $value) {
            ...
            ?>
                        <a style="display:none;" tabindex="-1" href="#" class="btn btn-pay select-payment <?php echo $active_payment; ?>" data-payment="<?php echo H($value); ?>">
                        <?php echo $value; ?>
                        </a>
                <?php } ?>
        ...
        <?php echo form_dropdown('payment_type', $payment_options, $selected_payment, 'id="payment_types" class="hidden"'); ?>
                    <?php echo form_input(array('name' => 'amount_tendered','style'=>'display:none;', 'id' => 'amount_tendered', 'value' => to_currency_no_money($amount_due), 'class' => 'add-input ')); ?>
                        <span class="input-group-addon">
...
<?php if (!$this->agent->is_mobile()) { ?>
    <?php
    if (!$this->config->item('auto_focus_on_item_after_sale_and_receiving')) {
        ?>
...
<?php if ($this->sale_lib->get_payment_amount(lang('common_store_account')) > 0) { ?>
    ...
    <?php
    } elseif ($is_over_credit_limit && $mode != 'store_account_payment' && $this->config->item('disable_store_account_when_over_credit_limit')) {
