...
<html>
    <head>
        <title><?php
            ...
            echo!is_on_demo_host() ? $this->config->item('company') . ' -- ' . lang('common_powered_by') . ' Baza App' : 'Demo - Baza App | Easy to use Online POS Software'
            ?></title>
        <meta charset="UTF-8" />
        ...
        <script src="<?php echo base_url(); ?>assets/js/jquery.js?<?php echo APPLICATION_VERSION; ?>" type="text/javascript" language="javascript" charset="UTF-8"></script>
        <style type="text/css">
            body
            {
                padding: 5px;
            ...
            $(document).ready(function ()
            {
                ...
                $(".checkForUpdate").click(function (event)
                {
                    ...
                    $.getJSON($(this).attr('href'), function (update_available)
                    {
                        ...
                        if (update_available)
                        {
                            $(".checkForUpdate").parent().html(<?php echo json_encode(lang('common_update_available') . ' <a href="http://baza.rw/downloads.php" target="_blank">' . lang('common_download_now') . '</a>'); ?>);
                        }
                        else
                        {
...
<?php echo form_open('login/index/'.$found_item_id, array('class' => 'form login-form', 'id' => 'loginform', 'autocomplete' => 'off')) ?>            
<p>
    <?php
...
<div class="bottom_info">
    ...
    if (!is_on_phppos_host()) {
        ?>
        <span><?php echo anchor('users/create_account', lang('common_create_account'), array('class' => 'pull-left')); ?></span>&nbsp;
...
<div class="version">
    <p>
        <?php echo date("Y") ?> <?php echo lang('login_version'); ?> <span class="badge bg-success"><?php echo APPLICATION_VERSION; ?></span>
    </p>
    ...
    <?php if (isset($subscription_payment_failed) && $subscription_payment_failed === true) { ?>
        ...
        <a class="btn btn-block btn-danger" href="http://baza.rw/update_billing.php" target="_blank"><?php echo lang('login_update_billing_info'); ?></a>
    <?php } ?>
