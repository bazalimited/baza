        ...
        <div class="panel panel-piluku">
            ...
            <div class="panel-body">
                ...
                <form  class="form-horizontal form-horizontal-mobiles">
                    ...
                    <div class="form-group">
                        <?php echo form_label(lang('reports_category') . ' :', 'category_input_data', array('class' => 'col-sm-3 col-md-3 col-lg-2 control-label  ')); ?> 
                        <div class="col-sm-9 col-md-2 col-lg-2">
                            <?php echo form_dropdown('category_input_data', $categories, '', 'class="form-control form-inps" id ="category_input_data"'); ?>
                        </div>
                    </div>
                    ...
                    <?php if (!$hide_excel_export_and_compare) { ?>
                        <div class="form-group">
...
<script type="text/javascript" language="javascript">
    $(document).ready(function ()
    {
        $("#supplier_input_data").select2(
                {
                    ...
                    ajax: {
                        url: <?php echo json_encode($supplier_search_suggestion_url); ?>,
                        dataType: 'json',
        ...
        $("#generate_report").click(function ()
        {
            ...
            var export_excel = 0;
            var category_id = -1
            var supplier_id = $("#supplier_input_data").val() ? $("#supplier_input_data").val() : -1;
            var category_id = $("#category_input_data").val() ? $("#category_input_data").val() : -1;
...
<?php if (!$hide_excel_export_and_compare) { ?>
                ...
                if ($("#simple_radio").prop('checked'))
                {
                    window.location = window.location + '/' + date_range_regular + '/' + do_compare + '/' + date_range_compare + '/' + supplier_id + '/' + category_id + '/' + sale_type + '/' + export_excel;
                }
                else
                {
                    var start_date = $("#start_date").val();
...
<?php } else { ?>
                ...
                if ($("#simple_radio").prop('checked'))
                {
                    window.location = window.location + '/' + $("#report_date_range_simple option:selected").val() + '/' + supplier_id + '/' + category_id + '/' + sale_type;
                }
                else
                {
                    var start_date = $("#start_date").val();
                    ...
                    window.location = window.location + '/' + start_date + '/' + end_date + '/' + supplier_id + '/' + category_id + '/' + sale_type;
                }
    <?php
}
?>
